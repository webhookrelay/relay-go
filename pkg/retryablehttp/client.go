...
func init() {
	...
	l, _ := cfg.Build()
	defaultClient = NewClient(l.Sugar())
}
...
// consumers.
type RequestLogHook func(*zap.SugaredLogger, *http.Request, int)
...
type ResponseLogHook func(*zap.SugaredLogger, *http.Response)
...
type Client struct {
	...
	Logger     *zap.SugaredLogger // Customer logger instance.
...
func getHeaderKey(k string) string {
	return fmt.Sprintf("X-WebhookRelay-%s", k)
}

func SetHeader(key, val string, resp *http.Response) {
	resp.Header.Set(getHeaderKey(key), val)
}

func GetHeader(key string, resp *http.Response) string {
	return resp.Header.Get(getHeaderKey(key))
}

func GetRetries(resp *http.Response) int {
	hdr := resp.Header.Get(getHeaderKey(HeaderRetries))
	if hdr == "" {
		return 0
	}

	ret, err := strconv.Atoi(hdr)
	if err != nil {
		return 0
	}
...
func (c *Client) Do(req *Request) (*http.Response, error) {
	...
	for i = 0; ; i++ {
		...
		if !checkOK {
			...
			SetHeader(HeaderRetries, strconv.Itoa(i), resp)
			return resp, err
		}
