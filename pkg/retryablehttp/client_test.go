...
func testClient_Do(t *testing.T, body interface{}) {
	...
	client := NewClient(zap.S())
	client.RetryWaitMin = 10 * time.Millisecond
	client.RetryWaitMax = 50 * time.Millisecond
	client.RetryMax = 50
	client.RequestLogHook = func(logger *zap.SugaredLogger, req *http.Request, retryNumber int) {
...
func TestClient_Get(t *testing.T) {
	...
	// Make the request.
	resp, err := NewClient(zap.S()).Get(ts.URL + "/foo/bar")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	resp.Body.Close()
...
func TestClient_RequestLogHook(t *testing.T) {
	...
	client := NewClient(zap.S())
	client.RequestLogHook = func(logger *zap.SugaredLogger, req *http.Request, retry int) {
		retries = retry
...
func TestClient_Head(t *testing.T) {
	...
	// Make the request.
	resp, err := NewClient(zap.S()).Head(ts.URL + "/foo/bar")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	resp.Body.Close()
...
func TestClient_PostForm(t *testing.T) {
	...
	// Make the request.
	resp, err := NewClient(zap.S()).PostForm(ts.URL+"/foo/bar", form)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	resp.Body.Close()
